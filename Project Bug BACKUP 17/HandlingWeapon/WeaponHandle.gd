...
var rand_rotation
var rotation_dir
# Varaiables for the slash attack
# -------------------------------
export(int) var slash_angle = 90 # In degrees, must be converted to radians
#export(float) var slash_time = 0.5
...
func _ready():
	slash_angle = deg2rad(slash_angle)
...
# -----------------------------------
func slash_attack_prepare(): # called at the first frame of the attack
	rotation_goal = global_rotation + (slash_angle / 2)
	rotation_dir = sign(rotation_goal)
	global_rotation = (get_local_mouse_position().angle() - (slash_angle / 2)) * rotation_dir
	center.global_rotation = (get_local_mouse_position().angle() - (slash_angle / 2)) * rotation_dir

func slash_attack(delta): # called every frame of the attack
	if abs(global_rotation - rotation_goal) > rotation_max_dist:
		global_rotation += slash_speed * delta
...
func SwordHitbox_area_entered(area):
	if area.is_in_group("EnemyHitbox"):
		if (weapon_states.state == weapon_states.states.moveing || weapon_states.state == weapon_states.states.call_back || weapon_states.state == weapon_states.states.attack):
