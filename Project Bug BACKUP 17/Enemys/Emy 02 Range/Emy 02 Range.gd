...
export(float) var acceleration = 140
export(float) onready var friction = max_speed / 2
...
export(int) var shoot_angle = 360  # in degrees
export(int) var numb_of_bullets = 10
export(float) var time_between_shots = 0.5
export(int) var radius_x = 100 # For bullet spawning
var radius = Vector2(radius_x, 0)
export(int) var max_shoot_distance = 1200
export(int) var min_shoot_distance = 400
...
#--------------------------------
func time_between_walk_timeout(delta):
...
func attack(delta):
	if player_node != null:
		...
		velocity = velocity.move_toward(move_direction * max_speed * attack_speed_multiplyer, acceleration * delta)
	else:
		is_attacking = false
		$Time_between_shots.set_paused(true)
...
func shoot_bullets():
	var angle_step
	if numb_of_bullets > 1:
		angle_step = shoot_angle / (numb_of_bullets - 1)
	else:
		angle_step = shoot_angle
#	center.rotation = center.global_position.direction_to(player_node.global_position).angle()
	center.look_at(player_node.global_position)
	
	for i in numb_of_bullets:
...
func retreat(delta):
	if player_node != null:
		move_direction = player_node.global_position.direction_to(self.global_position).normalized()
		x_dir = move_direction.round().x # for turning
		velocity = velocity.move_toward(move_direction * max_speed * attack_speed_multiplyer, acceleration * delta)
